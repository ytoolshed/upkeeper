...
static v0_svcinfo_t    *deserial_svcinfo_data(upk_pkt_buf_t * UPK_BUF); /* A helper function for deserial_repl_svcinfo */
...
static upk_pkt_buf_t   *serial_svcinfo_data(v0_svcinfo_t * data, size_t UPK_DATA_LEN);  /* A helper function for
                                                                                         * serial_repl_svcinfo */
...
/* *******************************************************************************************************************
 * dispatch tables for static function selection; avoids both the wasted time (assuming compiler optimization doesn't
...
deserialize_req_payload(upk_pkt_buf_t * UPK_BUF)
{
    ...
    msgtype = (upk_req_msgtype_t) enum_buf;
...
deserialize_repl_payload(upk_pkt_buf_t * UPK_BUF)
{
    ...
    memcpy(&enum_buf, UPK_BUF, sizeof(enum_buf));
    msgtype = (upk_repl_msgtype_t) enum_buf;
...
deserialize_pub_payload(upk_pkt_buf_t * UPK_BUF)
{
    ...
    memcpy(&enum_buf, UPK_BUF, sizeof(enum_buf));
    msgtype = (upk_pub_msgtype_t) enum_buf;
...
{
    return (serial_pub_dispatch[((upk_publication_t *) UPK_DATA_PTR)->msgtype]) (UPK_DATA_PTR, UPK_DATA_LEN);
}
...
deserial_req_seq_start(upk_pkt_buf_t * UPK_BUF)
{
    ...
    UPK_FETCH_SCALAR(msg_seq_count);
...
deserial_req_action(upk_pkt_buf_t * UPK_BUF)
{
    ...
    UPK_FETCH_SCALAR(svc_id_len);
    UPK_FETCH_STRING(svc_id, svc_id_len);
    UPK_FETCH_SCALAR(action_len);
    UPK_FETCH_STRING(action, action_len);
...
{
    UPK_INIT_DESERIALIZE_MSG(v0_subscribe_req_t);
...
{
    UPK_INIT_DESERIALIZE_MSG(v0_unsubscribe_req_t);
...
{
    UPK_INIT_DESERIALIZE_MSG(v0_disconnect_req_t);
...
deserial_repl_result(upk_pkt_buf_t * UPK_BUF)
{
    ...
    UPK_FETCH_SCALAR(msg_len);
    UPK_FETCH_STRING(msg, msg_len);
...
deserial_svcinfo_data(upk_pkt_buf_t * UPK_BUF)
{
    ...
    UPK_FETCH_SCALAR(last_action_time);
    UPK_FETCH_SCALAR(last_action_status);
    UPK_FETCH_SCALAR(last_action_name_len);
    UPK_FETCH_STRING(last_action_name, last_action_name_len);
    UPK_FETCH_SCALAR(last_signal_time);
    UPK_FETCH_SCALAR(last_signal_status);
    UPK_FETCH_ENUM(upk_signal_name_t, last_signal_name);
    UPK_FETCH_SCALAR(buddy_pid);
    UPK_FETCH_SCALAR(proc_pid);
...
{
    UPK_INIT_DESERIALIZE_MSG(upk_publication_t);
...
{
    UPK_INIT_DESERIALIZE_MSG(upk_cancelation_t);
...
serial_req_action(void *UPK_DATA_PTR, size_t UPK_DATA_LEN)
{
    ...
    UPK_PUT_SCALAR(svc_id_len);
    UPK_PUT_STRING(svc_id);
    UPK_PUT_SCALAR(action_len);
...
{
    UPK_INIT_SERIALIZE_MSG(v0_subscribe_req_t);
...
{
    UPK_INIT_SERIALIZE_MSG(v0_unsubscribe_req_t);
...
{
    UPK_INIT_SERIALIZE_MSG(v0_disconnect_req_t);
...
serial_svcinfo_data(v0_svcinfo_t * UPK_DATA_PTR, size_t UPK_DATA_LEN)
{
    ...
    UPK_PUT_SCALAR(last_action_time);
    UPK_PUT_SCALAR(last_action_status);
    UPK_PUT_SCALAR(last_action_name_len);
    UPK_PUT_STRING(last_action_name);
    UPK_PUT_SCALAR(last_signal_time);
    UPK_PUT_SCALAR(last_signal_status);
    UPK_PUT_ENUM(last_signal_name);
    UPK_PUT_SCALAR(buddy_pid);
    UPK_PUT_SCALAR(proc_pid);
...
{
    UPK_INIT_SERIALIZE_MSG(upk_publication_t);
...
{
    UPK_INIT_SERIALIZE_MSG(upk_cancelation_t);
