...
When the Buddy Controller starts up (usually at system reboot), it sifts 
through the upkeeper data store and figures out which application
processes need to be started. If an application's actual state
is down, but its desired state is up, the buddy controller restarts it with 
a buddy process.
...
retries, it forces the application to stop via SIGKILL. Once it has
accomplished stopping the application, it marks its new actual
state in the upkeeper data store and exits.
...
if they're marked running in the data store. This is necessary as system
...
upkeeper-buddy-controller keeps running as a daemon at all times and gets
notified by the buddies if any of their controlled applications was shut down
or crashed. In this case, the buddy receives a SIGCHLD signal (or the blocking
waitpid() triggers) and marks the application as 'stopped' in the data store.
