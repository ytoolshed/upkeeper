...
which application
processes need to be started. If an application's actual state is down, but its
desired state is up, the buddy controller restarts it with a buddy process.
...
it marks the application as 'stopped' in the data store.
This also sends a SIGHUP signal to the buddy controller via a SQLite 
trigger, which causes teh controller to initiate a new scan of the
services table, finding out which need to be restarted.
